...
**NM-EPD-420** 是一款基于 ESP32-S3 的 4.2 英寸电子墨水屏显示开发板。它配备了三色（黑/白/红）、双色（黑/白）或四色（黑/白/红/黄）电子纸面板、音频编解码器（ES8311）、环境传感器（AHT20）、RGB LED（WS2812）、SD 卡槽和 LoRa 模组（Heltec RA62） —— 非常适合低功耗物联网仪表盘、户外标识和智能标签等应用场景。
...
| 项目 | 规格 |
|------|------|
| **面板** | GDEY042Z98 三色电子墨水屏、GYE042A87 黑白电子墨水屏或 GDEY0420F51 四色电子墨水屏，相同的引脚定义 |
| **尺寸** | 4.2 英寸 |
| **分辨率** | 400 × 300 |
| **颜色** | 三色（黑/白/红）、双色（黑/白）或四色（黑/白/红/黄） |
| **驱动** | GxEPD2 |
| **接口** | SPI (FSPI) |

当前NM-EPD-420支持三色墨水屏（GDEY042Z98）、黑白墨水屏（GYE042A87）和四色墨水屏（GDEY0420F51），三者的刷新性能如下：
...
- **GDEY0420F51 四色墨水屏**：
  - **SKU： NM-EPD-420-4C**
  - 全刷（黑/白/红/黄）约 25-30 秒，不支持局部刷新
  - 对于需要更多颜色且不需要频繁刷新内容的应用，建议使用四色墨水屏（GDEY0420F51）。
  - PlatformIO 构建环境：`nm-epd-420-4c`；GxEPD2 驱动：`GxEPD2_420c_GDEY0420F51`（400 × 300）。与三色和黑白面板不同，四色驱动的 BUSY 信号为低电平有效。
